<?php
  ...
  try {
    $dsn = 'mysql:host=localhost;dbname=quiz2';
    $username = 'phpmyadmin';
    $password = 'GR3ATN3SS$';
    ...
    // Retrieve JSON data from the MBE course
    $sql = "SELECT JSON_OBJECT('MBE_course',course_json) FROM courses WHERE title = 'MBE';";
    $stmt = $pdo->query($sql);

    $data1 = $stmt->fetch(PDO::FETCH_ASSOC);
    
    $json = $data1['JSON_OBJECT(\'MBE_course\',course_json)'] ;
    ...
    // Insert lecture data into the lectures_archive table
    foreach ($data['MBE_course']['MBE_course'][0]['Lectures'] as $title => $lecture) {
        // Insert data into lectures_archive table
        // print_r($lecture['title']."<br>");
        // print_r($lecture['description']."<br>");
        // print_r($lecture['link'][0]."<br>");
        ...
        $sql = "INSERT INTO lectures_archive (course_id, title, description, slideshow_link) VALUES 
        (56412, '".$lecture['title']."', '".$lecture['description']."', '".$lecture['link'][0]."');";
        $pdo->query($sql);
    }
